...
//                }
                ...
                // RestController.java
                if (null != tableInfo.getRestControllerName() && null != pathInfo.get(ConstVal.CONTROLLER_PATH)) {
                    String restControllerFile = String.format((pathInfo.get(ConstVal.CONTROLLER_PATH) + File.separator + tableInfo.getRestControllerName() + this.suffixJavaOrKt()), entityName);
                    if (this.isCreate(restControllerFile)) {
                        this.writer(objectMap, this.templateFilePath(template.getRestController()), restControllerFile);
                    }
                }
            }
        } catch (Exception e) {
